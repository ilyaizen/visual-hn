...
def test_rewrite_proxy_header_brands_reader_and_repoints_link():
    ...
    # The h1 link is rebranded to plain "visual-hn" text.
    assert "vhn-rainbow" not in rewritten
    assert "vhn-logo-suffix" not in rewritten
    assert ">visual-hn</a>" in rewritten
...
def test_content_script_defaults_to_small_images_and_uses_theme_safe_settings():
    ...
    assert "--vhn-thumb-aspect: 4 / 3;" in css
